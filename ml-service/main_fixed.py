...
async def predict(input_data: MLInput):
    ...
    try:
        ...
        if prediction_label.lower() == 'fresh':
            freshness_score = 0.7 + (probabilities[prediction] * 0.3)  # 0.7-1.0 for fresh
            shelf_life_hours = 48 + (probabilities[prediction] * 24)   # 48-72 hours
        else:  # Spoiled
            freshness_score = probabilities[prediction] * 0.3  # 0.0-0.3 for spoiled
            shelf_life_hours = probabilities[prediction] * 12  # 0-12 hours
